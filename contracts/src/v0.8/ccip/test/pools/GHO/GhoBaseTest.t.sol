...
abstract contract GhoBaseTest is BaseTest {
  ...
  function _deployUpgradeableBurnMintTokenPool(
    ...
    UpgradeableBurnMintTokenPool tokenPoolImpl = new UpgradeableBurnMintTokenPool(ghoToken, 18, rmn, false);
  ...
  function _deployUpgradeableLockReleaseTokenPool(
    ...
    UpgradeableLockReleaseTokenPool tokenPoolImpl = new UpgradeableLockReleaseTokenPool(ghoToken, 18, rmn, false, true);
    // proxy deploy and init
    address[] memory emptyArray = new address[](0);
    bytes memory tokenPoolInitParams = abi.encodeWithSignature(
  ...
  function _enableLane(UtilsStorage storage s, uint256 fromId, uint256 toId) internal {
    ...
    remotePoolAddresses[0] = abi.encode(s.pools[toId]);
    chainUpdate[0] = UpgradeableTokenPool.ChainUpdate({
      remoteChainSelector: uint64(toId),
      remotePoolAddresses: remotePoolAddresses,
      remoteTokenAddress: abi.encode(s.tokens[toId]),
      outboundRateLimiterConfig: emptyRateConfig,
      inboundRateLimiterConfig: emptyRateConfig
    });

    vm.startPrank(AAVE_DAO);
    UpgradeableTokenPool(s.pools[fromId]).applyChainUpdates(new uint64[](0), chainUpdate);
    remotePoolAddresses[0] = abi.encode(s.pools[fromId]);
    chainUpdate[0].remoteChainSelector = uint64(fromId);
    chainUpdate[0].remotePoolAddresses = remotePoolAddresses;
    chainUpdate[0].remoteTokenAddress = abi.encode(s.tokens[fromId]);

    UpgradeableTokenPool(s.pools[toId]).applyChainUpdates(new uint64[](0), chainUpdate);
    vm.stopPrank();
  }
  ...
  function _moveGhoOrigin(
    ...
    vm.startPrank(RAMP);
    IPoolV1(s.pools[fromChain]).lockOrBurn(
      Pool.LockOrBurnInV1({
        receiver: abi.encode(user),
        remoteChainSelector: uint64(toChain),
        originalSender: user,
        amount: amount,
        localToken: s.tokens[fromChain]
      })
    );
  ...
  function _moveGhoDestination(
    ...
    vm.startPrank(RAMP);
    IPoolV1(s.pools[toChain]).releaseOrMint(
      Pool.ReleaseOrMintInV1({
        originalSender: abi.encode(user),
        remoteChainSelector: uint64(fromChain),
        receiver: user,
        amount: amount,
        localToken: s.tokens[toChain],
        sourcePoolAddress: abi.encode(s.pools[fromChain]),
