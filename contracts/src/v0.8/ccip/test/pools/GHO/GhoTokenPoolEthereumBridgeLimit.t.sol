...
contract GhoTokenPoolEthereumBridgeLimitSimpleScenario is GhoTokenPoolEthereumBridgeLimitSetup {
  ...
  function testFuzz_BridgeTwoSteps(uint256 amount1, uint256 amount2) public {
    ...
    if (amount1 + amount2 > maxAmount) {
      vm.expectRevert();
      vm.prank(RAMP);
      IPoolV1(s.pools[0]).lockOrBurn(
        Pool.LockOrBurnInV1({
          receiver: bytes(""),
          remoteChainSelector: uint64(1),
          originalSender: USER,
          amount: amount2,
          localToken: s.tokens[0]
        })
      );
  ...
  function testFuzz_BridgeBackAndForth(uint256 amountOut, uint256 amountIn) public {
    ...
    if (amountIn > amountOut) {
      ...
      vm.expectRevert();
      vm.prank(RAMP);
      IPoolV1(s.pools[0]).releaseOrMint(
        Pool.ReleaseOrMintInV1({
          ...
          amount: amountIn,
          localToken: s.tokens[0],
          sourcePoolAddress: abi.encode(address(s.pools[0])),
  ...
  function testFuzz_BridgeCapacityExceededDestinationReverts(uint256 amount, uint256 level) public {
    ...
    vm.prank(RAMP);
    IPoolV1(s.pools[1]).releaseOrMint(
      Pool.ReleaseOrMintInV1({
        originalSender: bytes(""),
        remoteChainSelector: uint64(0),
        receiver: USER,
        amount: amount,
        localToken: s.tokens[1],
        sourcePoolAddress: abi.encode(address(s.pools[0])),
        sourcePoolData: bytes(""),
        offchainTokenData: bytes("")
      })
  ...
  function testFuzz_BridgeBackZeroBridgeLimitDestinationReverts(uint256 amount, uint256 bridgeAmount) public {
    ...
    IPoolV1(s.pools[0]).releaseOrMint(
      Pool.ReleaseOrMintInV1({
        ...
        amount: amount,
        localToken: s.tokens[0],
        sourcePoolAddress: abi.encode(address(s.pools[1])),
        sourcePoolData: bytes(""),
        offchainTokenData: bytes("")
      })
    );
  ...
  function testIncreaseBucketCapacity() public {
    ...
    vm.expectRevert(abi.encodeWithSelector(BridgeLimitExceeded.selector, INITIAL_BRIDGE_LIMIT));
    vm.prank(RAMP);
    IPoolV1(s.pools[0]).lockOrBurn(
      Pool.LockOrBurnInV1({
        receiver: bytes(""),
